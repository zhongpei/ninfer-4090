...
struct Fixture {
    ...
    Fixture() : payload(1344) {
        // mkdtemp is POSIX; create_directory reports whether it made a new directory, which gives
        // the same exclusive-creation guarantee on every platform.
        std::random_device entropy;
        for (int attempt = 0;; ++attempt) {
            const auto candidate = std::filesystem::temp_directory_path() /
                                   ("ninfer-artifact-" + std::to_string(entropy()) + "-" +
