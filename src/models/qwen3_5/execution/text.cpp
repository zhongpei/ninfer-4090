    ...
    if (rope_positions == nullptr) {
        ...
        if (rope_scaling_factor_ > 1.0F) {
            ops::scale_positions_yarn(
                generated_rope_positions,
                static_cast<std::uint32_t>(rope_scaling_original_context_),
...
void TextContext::mtp_forward_ar_step(const Tensor& token, const Tensor& previous_hidden,
    ...
    if (rope_scaling_factor_ > 1.0F) {
        ops::scale_positions_yarn(
            rope_position, static_cast<std::uint32_t>(rope_scaling_original_context_),
...
TextContext::prefill_impl(std::span<const int> ids, const TextPrefill* text_prefill,
    ...
    for (; t0 < T;) {
...

        {
            ...
            // YaRN must never overwrite raw cache positions. Allocate a distinct RoPE buffer
            // whenever scaling is enabled, even when the sequence has no multimodal rope delta.
            ...
            if (rope_scaling_factor_ > 1.0F) {
                // Vision prefill stores MRoPE as a contiguous [T,3] matrix. The YaRN ramp is
                // elementwise, so flatten either the text [T] or multimodal [T,3] view.
