...
struct alignas(16) PackedAttentionTile {
    std::int32_t q0;
    std::int32_t begin;
    std::int32_t end;
...
static_assert(sizeof(PackedAttentionTile) == 16);

__device__ __forceinline__ const __nv_bfloat16*
packed_attention_ptr(const __nv_bfloat16* data, std::int64_t stride_d, std::int64_t stride_h,
...
__device__ __forceinline__ int packed_attention_swz(int row, int col) {
    return (((col >> 3) ^ (row & 7)) << 3) | (col & 7);
}

__device__ __forceinline__ unsigned packed_attention_swz_addr(unsigned lane_base, unsigned ck,
                                                              unsigned as, unsigned r) {
    return lane_base + ((ck | as) ^ r);
}

__global__ void packed_attention_prepare_tiles_kernel(const std::int32_t* cu_seqlens,
                                                      std::int32_t segments,
                                                      PackedAttentionTile* tiles,
    ...
    for (int segment = 0; segment < segments; ++segment) {
        ...
        for (int q0 = begin; q0 < end && next < max_tiles; q0 += kPackedAttentionBr) {
...
packed_attention_stage_q(__nv_bfloat16* dst, const __nv_bfloat16* q, int q0, int end, int head,
    ...
    constexpr int VecsPerRow = kPackedAttentionPaddedD / 8;
...
#pragma unroll
        ...
        ldmatrix_x4(af[0][0], af[0][1], af[0][2], af[0][3],
                    packed_attention_swz_addr(q_lane_base, 0u, q_as, q_r));
#pragma unroll
        for (int nt2 = 0; nt2 < QKNt; nt2 += 2) {
            ldmatrix_x4(
                bf[0][nt2][0], bf[0][nt2][1], bf[0][nt2 + 1][0], bf[0][nt2 + 1][1],
                packed_attention_swz_addr(k_lane_base + static_cast<unsigned>(nt2 * 8 * RowBytes),
                                          0u, k_as, k_r));
        }
#pragma unroll
        for (int ks = 0; ks < QKKs; ++ks) {
            ...
            if (ks + 1 < QKKs) {
                ...
                ldmatrix_x4(af[nxt][0], af[nxt][1], af[nxt][2], af[nxt][3],
                            packed_attention_swz_addr(q_lane_base, ck, q_as, q_r));
#pragma unroll
                for (int nt2 = 0; nt2 < QKNt; nt2 += 2) {
                    ldmatrix_x4(bf[nxt][nt2][0], bf[nxt][nt2][1], bf[nxt][nt2 + 1][0],
                                bf[nxt][nt2 + 1][1],
                                packed_attention_swz_addr(
...
#pragma unroll
        ...
        ldmatrix_x4_t(vf[0][0], vf[0][1], vf[0][2], vf[0][3],
                      packed_attention_swz_addr(v_lane_base, 0u, v_as, v_r));
#pragma unroll
