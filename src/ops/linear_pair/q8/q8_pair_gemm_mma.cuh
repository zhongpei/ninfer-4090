...
__global__
__launch_bounds__((TileCols / 16) * 32, kQ8PairMmaMinBlocks<TileCols>) void q8_pair_gemm_mma_kernel(
...
#pragma unroll
            for (int gg = 0; gg < 2; ++gg) {
                ...
                store_vec(&As[row * BK + q8_g32_swz64(row, col)], values);
            }
        }
    };
...
#pragma unroll
            for (int ni = 0; ni < NT; ++ni) {
                ...
                ldmatrix_x2(bf[slot][ni][0], bf[slot][ni][1],
                            smem_addr(&Bs[stage][br * BK + q8_g32_swz64(br, bc)]));
            }
        };
        load_fragments(0, 0);
#pragma unroll
