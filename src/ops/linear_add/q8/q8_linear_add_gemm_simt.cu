...
void launch_decode(const Tensor& x, const Weight& w, Tensor& residual_out, cudaStream_t stream) {
    ...
    // q8_linear_add_decode_kernel bakes K into kDecodeK: it walks exactly 6144 activations and
    ...
    if (x.ne[0] != kDecodeK || w.k != kDecodeK || w.padded_shape[1] != kDecodeK) {
        throw std::invalid_argument("q8 linear_add decode: kernel is specialised for K=6144");
    }
    if (x.ne[1] != 1 || residual_out.ne[1] != 1) {
        throw std::invalid_argument("q8 linear_add decode: kernel requires a single column");
    }
